...
- September 23–24 NVL72 follow-up: TCO flags GB200/GB300 rack-scale infrastructure in both workload and spend modes, including a single NVL72. A user-supplied project quote or facility-coverage reference is tied to the selected cost basis but never independently verified; recording one keeps the result and report directional. The old eight-GPU Equinix bundle cannot serve as an NVL72 review basis without a positive, distinct override, which remains user supplied. Rack-scale retrofit amounts accept direct quote input, and five-year scenarios disclose that renewal after an initial three-year system term is not modeled. Rubin Phase 1 remains provisional; no high-density rack BOM is invented. Smaller B200/B300 planning behavior is preserved.
