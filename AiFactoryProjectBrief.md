...
- September 23 NVL72 follow-up: TCO now flags GB200/GB300 rack-scale infrastructure in both workload and spend modes, including a single NVL72. A user-supplied project quote or documented facility-coverage reference can qualify the scenario's existing planning inputs after rack/cooling/power distribution/fabric/installation/software/facility costs are reviewed; the attestation is tied to the cost basis and is not independently verified. The old eight-GPU Equinix bundle cannot qualify an NVL72 without a project-specific override. Rubin Phase 1 remains provisional even if a reference is supplied; no high-density rack BOM is invented. Smaller B200/B300 planning behavior is preserved.
