...
text = text.replace('      sourceClass, gpuSizingCount, workingDayHours,\n', '      sourceClass, gpuSizingCount, workingDayHours,\n', 1)
text = text.replace('      sourceUseCase, modelAdvisorRecommendedId,\n', '      sourceUseCase, modelAdvisorRecommendedId,\n', 1)
...
if end == -1: raise SystemExit('preview block end missing')
...
text = text.replace('// Minimum viable spend: smallest monthly bill where on-prem beats cloud at the selected horizon (bake-off mode only -- workload mode\'s cost isn\'t driven by the bill, so this question doesn\'t apply there)', '// Minimum viable spend: smallest monthly bill where on-prem beats cloud at the selected horizon (bake-off mode only -- workload mode\'s cost isn\'t driven by the bill, so this question doesn\'t apply there)')
