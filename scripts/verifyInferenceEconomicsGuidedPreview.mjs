...
assert.ok(guided.includes("Save to PDF"));
assert.ok(guided.includes("Calculation Methodology & Audit Trail"));
assert.ok(guided.includes("Private cost / 1M output = assigned private TCO"));
assert.ok(guided.includes("Managed API cost / 1M output = output rate"));
assert.ok(guided.includes("ie-print-summary"));
