...
for (const [metric, priorities] of Object.entries(ACCEPTED_MARGINS))
  for (const [priority, expected] of Object.entries(priorities))
    assert(MARGINS[metric]?.[priority] === expected, `Live Advisor margin drift for ${metric}/${priority}.`);
...
assert(
  qwen?.intelligence_index === 41.4 && qwen?.coding_index === 68.1 && qwen?.agentic_index === 46.8,
  "Qwen3.8 AA v4.3 mapping drifted."
);
const gemma = production.find((m) => m.canonical_model_id === "gemma-4-26b-a4b-it");
assert(
  gemma?.intelligence_index === 13.9 && gemma?.coding_index == null && gemma?.agentic_index == null,
  "Gemma 4 default-semantics AA mapping drifted."
);
for (const id of ["deepseek-v4-flash-0731", "deepseek-v4-pro-0813"]) {
  const m = production.find((x) => x.canonical_model_id === id);
  assert(
    m && m.intelligence_index == null && m.coding_index == null && m.agentic_index == null,
    `${id} must remain scoreless until an approved default-semantics AA mapping exists.`
  );
}
const nemotron = production.find((m) => m.canonical_model_id === "nemotron-3-super-120b-a12b");
assert(
  nemotron?.intelligence_index === 18.6 && nemotron?.coding_index === 37.7 && nemotron?.agentic_index === 4.2,
  "Nemotron AA mapping drifted."
);
...
for (const [workload, metric] of Object.entries(metricByWorkload)) {
  for (const quality of ["frontier-like", "strong", "economical"]) {
    ...
    assert(
      result.bestPerformance?.canonical_model_id === "qwen3.8-27b",
      `${workload}/${quality}: Qwen3.8 must be Best Performance under the current v4.3 evidence snapshot.`
    );
    assert(
      result.balanced?.canonical_model_id === "qwen3.8-27b",
      `${workload}/${quality}: Qwen3.8 must remain the balanced choice under the current v4.3 evidence snapshot.`
    );
    assert(
      result.efficiency?.canonical_model_id === "qwen3.8-27b",
      `${workload}/${quality}: Qwen3.8 must remain the efficiency choice under the current v4.3 evidence snapshot.`
    );
  }
}

console.log(
  "Model Advisor activated calibration PASS: accepted margins remain exact across the 13-model recommended population; the approved AA v4.3 mappings make Qwen3.8 the current unconstrained intelligence/coding/agentic leader and all three ranking slots; Gemma 4 carries intelligence-only default-semantics evidence; DeepSeek V4 Flash/Pro remain deliberately scoreless."
);
