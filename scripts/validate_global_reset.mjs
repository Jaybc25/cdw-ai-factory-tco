...
need(auth, "if (!session?.user?.id || snapshotWritesBlockedRef.current) return;", "saveSnapshot must refuse stale writes during reset");
...
need(readiness, 'window.localStorage.removeItem("cdw-readiness")', "Empty Readiness state must not recreate persistence after reset");
