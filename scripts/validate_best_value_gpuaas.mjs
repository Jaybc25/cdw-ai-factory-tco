...
assert.deepEqual(
  ...
  "Customer-facing provider list should use clear enabled provider names"
);
assert.equal(
  GPUAAS_PROVIDER_CONFIG.CoreWeave.customerFacingEnabled,
  false,
  "CoreWeave must be hidden from customer-facing provider choices until commercially enabled"
);
assert.equal(
  GPUAAS_PROVIDER_CONFIG.CoreWeave.bestValueEnabled,
  false,
  "CoreWeave must remain disabled for Best-Value recommendations until commercially enabled"
);
assert.ok(
  CLOUD_GPU_RATES.CoreWeave?.H200,
  "CoreWeave modeling data should remain preserved internally for future re-enablement"
);
assert.strictEqual(
  CLOUD_GPU_RATES["Google Cloud"],
  CLOUD_GPU_RATES.GCP,
  "Google Cloud display name should resolve to the canonical GCP rate data"
);
assert.strictEqual(
  CLOUD_GPU_RATES["Oracle Cloud"],
  CLOUD_GPU_RATES.OCI,
  "Oracle Cloud display name should resolve to the canonical OCI rate data"
);
...
assert.ok(
  !h200.some((row) => row.provider === "CoreWeave"),
  "Commercially disabled providers must not enter Best-Value rankings"
);
assert.ok(
  h200.every((row) => row.gpuClass === "H200"),
  "v1 must never substitute GPU classes"
);
...
assert.equal(
  b300.find((row) => row.provider === "Google Cloud")?.confidence,
  "QUOTE",
  "Google Cloud B300 must retain its QUOTE confidence"
);
assert.equal(
  b300.find((row) => row.provider === "Oracle Cloud")?.confidence,
  "LISTED",
  "Oracle Cloud B300 must retain its LISTED confidence"
);
...
assert.deepEqual(
  explicitDisabledCandidate.map((row) => row.provider),
  ["AWS"],
  "Ranking helper must reject an explicitly supplied but commercially disabled provider"
);
...
assert.deepEqual(
  tie.map((row) => row.provider),
  ["A", "B"],
  "Equal modeled costs should sort deterministically by provider name"
);
