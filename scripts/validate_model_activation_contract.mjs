...
for (const id of ACTIVE) {
  ...
  if (HYBRID_SOURCE.has(id)) {
    ...
    assert(source && source.sequenceStateType === runtimeById.get(id).sequenceStateType, `${id} promoted hybrid runtime drifted from source-qualified methodology.`);
  }
}

const nemotron = advisorById.get("nemotron-3-super-120b-a12b");
assert(capById.get("nemotron-3-super-120b-a12b")?.confidence === "HIGH", "Nemotron must retain HIGH-confidence AA mapping.");
const permissive = applyHardFilters([nemotron], { license: "permissive-commercial", governance: "none", contextWindow: "8k", multimodal: "any" })[0];
assert(permissive.filterDetails.licenseState === "PASS", "Nemotron NVIDIA open-model license must pass permissive-commercial filtering after explicit heuristic support.");
