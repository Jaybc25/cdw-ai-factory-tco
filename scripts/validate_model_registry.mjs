...
import {
  MODEL_REGISTRY, MODEL_ARCHITECTURE_SCHEMA_VERSION, MODEL_ARCHITECTURE_TYPES,
  DEFAULT_MODEL_ID, RECOMMENDED_MODELS, EXISTING_DEPLOYMENT_MODELS,
  getModelById, getVisibleModelOptions, isRecommendedModel,
...
function diff(a, b) { return [...a].filter((id) => !b.has(id)); }

const missing = diff(catalogIds, registryIds); const extra = diff(registryIds, catalogIds);
...
for (const model of MODEL_REGISTRY) {
  if (model.schemaVersion !== MODEL_ARCHITECTURE_SCHEMA_VERSION) throw new Error(`Model ${model.id} schema version mismatch.`);
  if (!architectureTypes.has(model.architectureType)) throw new Error(`Model ${model.id} has invalid architectureType.`);
  if (!(Number.isFinite(model.totalParamsB) && model.totalParamsB > 0 && Number.isFinite(model.activeParamsB) && model.activeParamsB > 0 && model.activeParamsB <= model.totalParamsB)) throw new Error(`Model ${model.id} has invalid total/active parameter semantics.`);
  ...
  if (sparse) {
    if (!(Number.isFinite(model.numExperts) && model.numExperts >= 1 && Number.isFinite(model.numSharedExperts) && model.numSharedExperts >= 0 && Number.isFinite(model.routedExpertsPerToken) && model.routedExpertsPerToken >= 1 && Number.isFinite(model.activeExpertsPerToken) && model.activeExpertsPerToken >= 1)) throw new Error(`Sparse model ${model.id} has incomplete expert-routing fields.`);
    if (model.routedExpertsPerToken > model.numExperts || model.activeExpertsPerToken !== model.routedExpertsPerToken + model.numSharedExperts) throw new Error(`Sparse model ${model.id} has inconsistent expert-routing semantics.`);
  } else if ([model.numExperts, model.numSharedExperts, model.routedExpertsPerToken, model.activeExpertsPerToken].some((v) => v != null)) throw new Error(`Dense model ${model.id} must not define MoE routing fields.`);

  if (!Array.isArray(model.modalities) || !model.modalities.length || !model.modalities.includes("text")) throw new Error(`Model ${model.id} must include text modality.`);
  for (const modality of model.modalities) if (!allowedModalities.has(modality)) throw new Error(`Model ${model.id} has unsupported modality ${modality}.`);
  if (!(typeof model.architectureSource === "string" && /^https:\/\//.test(model.architectureSource))) throw new Error(`Model ${model.id} lacks architecture provenance.`);
  if (!allowedStatuses.has(model.catalogStatus)) throw new Error(`Model ${model.id} has invalid resolved status ${model.catalogStatus}.`);
  if (getModelResidencyParamsB(model) !== model.totalParamsB || getModelActiveParamsB(model) !== model.activeParamsB) throw new Error(`Parameter helpers drifted for ${model.id}.`);
  ...
  if (Number.isFinite(catalogParams) && Math.abs(model.totalParamsB - catalogParams) / catalogParams > 0.05) throw new Error(`Technical parameters for ${model.id} differ from Advisor specs by >5%.`);
}

const defaultVisible = getVisibleModelOptions();
if (defaultVisible.some((m) => m.catalogStatus === "existing-deployment")) throw new Error("Default visible list exposed existing-deployment model.");
if (!defaultVisible.some((m) => m.id === "custom")) throw new Error("Custom model must remain available.");
for (const model of RECOMMENDED_MODELS) if (!defaultVisible.some((m) => m.id === model.id)) throw new Error(`Recommended model ${model.id} missing from default visible list.`);
const withExisting = getVisibleModelOptions({ includeExisting: true });
for (const model of EXISTING_DEPLOYMENT_MODELS) if (!withExisting.some((m) => m.id === model.id)) throw new Error(`Existing-deployment toggle missing ${model.id}.`);

const advisorResult = buildRecommendations(getCatalog(), { license: "need-to-check", governance: "none", contextWindow: "8k", multimodal: "any", primaryWorkload: "rag", qualityPriority: "strong", optimizationPriority: "balanced" });
const advisorSurfaceIds = new Set([...advisorResult.cards, ...advisorResult.otherEligible, ...advisorResult.verificationCandidates].map((x) => x.model?.canonical_model_id || x.canonical_model_id));
