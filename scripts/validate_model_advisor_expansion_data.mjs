...
const DELIBERATELY_UNMAPPED_STAGE_IDS = new Set(["deepseek-v4-flash-0731", "deepseek-v4-pro-0813"]);
...
for (const id of DELIBERATELY_UNMAPPED_STAGE_IDS) {
  ...
  if (canonicalEntry.aliases?.artificial_analysis_slug != null) {
    throw new Error(
      `${id} unexpectedly acquired an Artificial Analysis alias; verify exact default-semantics variant before mapping.`
    );
  }
  if (capById.has(id)) {
