...
const out = process.argv[2] || "artifacts/roi-report-experiment.pdf";
fs.mkdirSync(out.split("/").slice(0, -1).join("/") || ".", { recursive: true });

const browser = await chromium.launch({ headless: true });
const page = await browser.newPage({ viewport: { width: 1280, height: 1000 } });
await page.goto("http://127.0.0.1:4173/roi", { waitUntil: "networkidle" });
await page.getByRole("button", { name: /Get the full ROI report/i }).click();
