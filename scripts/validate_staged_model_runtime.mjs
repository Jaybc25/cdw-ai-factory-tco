...
function approx(a, e, t = 1e-9) { if (Math.abs(a - e) > t) throw new Error(`Expected ${e}; received ${a}.`); }

const trancheIds = [
  "granite-4.2-30b", "gpt-oss-20b", "gpt-oss-120b", "gemma-4-26b-a4b-it", "mistral-small-4", "mistral-large-3",
...
for (const id of trancheIds) {
  ...
  assert(getInferenceSequenceStateMemory(model, 8192, 2).bytesPerSequence > 0, `${id} production sequence-state contract failed.`);
...
for (const id of hybridIds) {
  ...
  assert(runtime.totalParamsB === source.totalParamsB && runtime.activeParamsB === source.activeParamsB && runtime.layers === source.layers, `${id} promoted core architecture fields drifted.`);
...
approx(nemotron.numExperts, 512); approx(nemotron.numSharedExperts, 1); approx(nemotron.routedExpertsPerToken, 22); approx(nemotron.activeExpertsPerToken, 23);
approx(nemotron.contextLength, 262144); approx(nemotron.advertisedExtendedContextLength, 1000000);
...
assert(STAGED_TECHNICAL_MODEL_REGISTRY.length === 4, "Four hybrid methodology/provenance source records must remain available after activation.");

console.log("Model runtime PASS: all ten tranche models are production-recommended; the four hybrid runtime records reconcile to source-qualified methodology fixtures, Nemotron routing/context semantics are explicit, and no unsupported throughput uplift is introduced.");
