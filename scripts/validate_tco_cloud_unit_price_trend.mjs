...
assert(
  Math.abs(trendCloudGpuCompute(100, 0.25, -0.1, 1) - 112.5) < 1e-9,
  "workload growth and price trend must compound independently"
);
assert(
  Math.abs(trendCloudGpuCompute(100, 0.25, 0.1, 1) - 137.5) < 1e-9,
  "positive price trend must compound with workload growth"
);
const s = fs.readFileSync("src/TcoCalculator.jsx", "utf8");
assert(
  s.includes("trendCloudGpuCompute(adjCloud.monthlyCompute"),
  "workload-mode cloud compute must use production price trend"
);
assert(
  s.includes("trendCloudGpuCompute(inp.bill * inp.computeShare"),
  "existing-spend mode must trend compute share only"
);
