...
const EXPECTED_HYBRID_SOURCE_IDS = new Set([
  "qwen3.8-27b",
...
assert(
  ...
  "Tranche manifest must record full 10-model activation."
);
assert(manifest.models.length === 10, "Tranche 1 must contain exactly 10 qualified models.");
assert(
  (policy.staged_models || []).length === 0,
  "No tranche model may remain staged in product policy after activation."
);
assert(
  STAGED_TECHNICAL_MODEL_REGISTRY.length === 4,
  "Four hybrid source records must remain available for methodology fixtures/provenance."
);
for (const source of STAGED_TECHNICAL_MODEL_REGISTRY)
  assert(EXPECTED_HYBRID_SOURCE_IDS.has(source.id), `Unexpected hybrid source record ${source.id}.`);
