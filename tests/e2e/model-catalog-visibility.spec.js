...
const ACTIVATED_MODEL_IDS = [
  "gemma-4-26b-a4b-it", "mistral-small-4", "mistral-large-3", "gpt-oss-20b", "gpt-oss-120b", "granite-4.2-30b",
];
const BLOCKED_MODEL_IDS = ["qwen3.8-27b", "deepseek-v4-flash-0731", "deepseek-v4-pro-0813", "nemotron-3-super-120b-a12b"];
const BLOCKED_MODEL_LABELS = ["Qwen3.8 27B", "DeepSeek V4 Flash 0731", "DeepSeek V4 Pro 0813", "NVIDIA Nemotron 3 Super 120B-A12B FP8"];
...
test("methodology-blocked tranche models remain absent from Model Advisor", async ({ page }) => {
  await page.goto("/model-advisor", { waitUntil: "domcontentloaded" });
  await expect(page.getByText(/Llama 4 Scout|Llama 4 Maverick|Meta Muse Glimmer 30B/).first()).toBeVisible();
  for (const label of BLOCKED_MODEL_LABELS) await expect(page.getByText(label, { exact: true })).toHaveCount(0);
});

test("GPU Sizing exposes all six activated models by default", async ({ page }) => {
  ...
  for (const id of ACTIVATED_MODEL_IDS) await expect.poll(() => page.locator(`option[value="${id}"]`).count()).toBeGreaterThan(0);
});
...
test("methodology-blocked models stay out of GPU Sizing even when existing deployments are enabled", async ({ page }) => {
  await page.goto("/gpu-sizing", { waitUntil: "domcontentloaded" });
  await page.getByLabel("Include models for existing deployments").check();
  for (const id of BLOCKED_MODEL_IDS) await expect(page.locator(`option[value="${id}"]`)).toHaveCount(0);
});
...
test("TCO exposes all six activated models under the shared current-model policy", async ({ page }) => {
...
test("methodology-blocked models stay out of TCO even when existing deployments are enabled", async ({ page }) => {
  await page.goto("/tco", { waitUntil: "domcontentloaded" });
  await openTcoCapacitySection(page);
  await page.getByLabel("Include models for existing deployments").check();
  for (const id of BLOCKED_MODEL_IDS) await expect(page.locator(`option[value="${id}"]`)).toHaveCount(0);
